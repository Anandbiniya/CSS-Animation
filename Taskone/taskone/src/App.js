...
export default class App extends Component {
  ...
  onDragOver = event => {
    ...
    const draggedPosition = event.dataTransfer.getData("text/plain").split(":")[1];
    const overPosition = getMobilePosition(event.clientY);
    if (draggedPosition !== overPosition) {
      const draggedIndex = mobiles.findIndex(m => m.position === parseInt(draggedPosition));
      const overIndex = mobiles.findIndex(m => m.position === parseInt(overPosition));
